...
if __name__ == '__main__':
    ...
    async def kick_inactive_users(context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if response.data:
            ...
            # Notify admins about the kicked users
            if kicked_users:
                admin_message = f"Kicked {len(kicked_users)} inactive users (31+ days since last purchase)."
            else:
                admin_message = "No inactive users to kick."
